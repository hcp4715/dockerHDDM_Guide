def _HDDMarviz(data=None,model=None,chains=4,draws=2000,warmup=1000,thin=1,nppc=1000,subsample=None,savefile=True,savetag=None):
    """
    ...
    if os.path.exists(InfDataName):
        ...
        ms_tmp = p_map(partial(model, 
                               df=data, 
                               draws=draws,
                               burn=warmup,    # here we have to use burn because it's the function in HDDM
                               thin=thin, 
...
def HDDMarviz(data=None,model_func=None,chains=4,draws=2000,warmup=1000,thin=1,nppc=1000,subsample=None,savefile=True,savetag=None):
    """
    ...
    draws: int, n of sample for mcmc
    nppc: # of posterior predictive check, default is 1000, if None, the number will be same as posterior draws, i.e., (draws - burn)/thin
